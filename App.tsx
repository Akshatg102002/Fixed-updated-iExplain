...
const CategoryTitleSlugWrapper = () => {
  ...
  const localStructuredCollegePage = STRUCTURED_COLLEGE_DETAILS[normalizedSlug]
    || Object.values(STRUCTURED_COLLEGE_DETAILS).find((item: any) => createSlug(item?.title || item?.name || '') === normalizedSlug);
  ...
  useEffect(() => {
    const loadLocalPage = () => {
      ...
      const localLegacyCollegePage = LEGACY_COLLEGE_DETAILS[normalizedSlug]
        || Object.values(LEGACY_COLLEGE_DETAILS).find((item: any) => createSlug(item?.title || item?.name || '') === normalizedSlug);

      if (localLegacyCollegePage) {
        setCollegePage(normalizeCollegeDetailData(
          localLegacyCollegePage,
          normalizedSlug
        ));
      } else {
        setCollegePage(null);
