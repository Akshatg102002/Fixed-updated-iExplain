...
const AdminPanel: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  ...
  const handleSyncLocalData = async () => {
    setLoading(true);
    try {
      const mbbsEntries = Object.entries(MBBS_ABROAD_DETAILED);
      const studyEntries = Object.entries(STUDY_ABROAD_DETAILED);

      for (const [slug, payload] of mbbsEntries) {
        await setDoc(
          doc(db, 'dynamic_pages', slug),
          {
      ...
      for (const [slug, payload] of studyEntries) {
        await setDoc(
          doc(db, 'dynamic_pages', slug),
          {
            title: (payload as any)?.title || '',
      ...
      await fetchData();
      alert('Local MBBS and Study Abroad page data synced successfully.');
    } catch (error) {
            ...
            <div className="animate-fade-in">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-3">
                  {viewMode === 'list' && (
                    <button onClick={handleSyncLocalData} className="px-6 py-3 bg-gray-100 text-brand-blue rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-gray-200 transition-all shadow-sm">
                      Sync Local Data
                    </button>
                  )}
                  <button onClick={() => { if(viewMode === 'list') { resetForms(); setViewMode('create'); } else { setViewMode('list'); } }} className="px-6 py-3 bg-brand-blue text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-brand-gold transition-all shadow-lg">
                ...
                <div className="bg-white rounded-[2.5rem] p-12 shadow-sm border border-gray-100">
                  <form onSubmit={(e) => { e.preventDefault(); handleSavePage(); }} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="label">SEO Focus Keyphrase</label>
